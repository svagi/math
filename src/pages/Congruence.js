...
export default class Congruence extends React.Component {
  state = {
    a: 0,
    x: 0,
    b: 0,
  ...
  render(state = this.state) {
    return (
      <div>
        ...
        <p>
          Formula: If <i>d | b</i> where <i>d = gcd(a, m)</i> then{' '}
          <i>a * x ≡ b (mod m)</i> has <i>d</i> solutions.
        </p>
        <form
        ...
        >
          ...
          <input
            ...
            required
          />
          <label htmlFor="number-a">Select number M</label>
          <input
            id="number-m"
            type="number"
            value={state.m || 0}
        ...
        <div style={style.calc}>
          ...
          <div>
            <i>
              {state.a} * x ≡ {state.b} (mod {state.m}) has{' '}
            </i>
            {state.x.length > 0 ? (
              <span>
                {state.x.length} solutions. <br />x ={' '}
                <strong>{state.x.join('; ')}</strong>
              </span>
            ) : (
